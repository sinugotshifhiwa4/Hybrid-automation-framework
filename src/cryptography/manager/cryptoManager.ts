...
export class CryptoManager {
  ...
  private static validateEncryptedFormat(encryptedData: string): void {
    if (!encryptedData.startsWith(SECURITY_CONSTANTS.FORMAT.PREFIX)) {
      ErrorHandler.logAndThrow('Invalid encrypted format: Missing prefix', 'CryptoManager.validateEncryptedFormat');
    }
  }
  ...
  public static validateBase64Components(
    ...
    if (!CryptoManager.isValidBase64(salt)) {
      ErrorHandler.logAndThrow('Invalid salt format', 'CryptoManager.validateBase64Components');
    }
    if (!CryptoManager.isValidBase64(iv)) {
      ErrorHandler.logAndThrow('Invalid IV format', 'CryptoManager.validateBase64Components');
    }
    if (!CryptoManager.isValidBase64(cipherText)) {
      ErrorHandler.logAndThrow('Invalid cipherText format', 'CryptoManager.validateBase64Components');
    }
    if (!CryptoManager.isValidBase64(receivedHmac)) {
      ErrorHandler.logAndThrow('Invalid HMAC format', 'CryptoManager.validateBase64Components');
    }
  }
